...
(() => {
  ...
  if (!pageUrl) return;
  const posts = (config.posts || []).map(publicFacebookUrl).filter(Boolean).slice(0, 3);
  ...
  const frames = (isCurated ? posts : [pageUrl]).map((url, index) => {
    ...
    frame.title = isCurated ? `Tynemouth Seal Hospital Facebook post ${index + 1}` : 'Tynemouth Seal Hospital Facebook Page timeline';
    frame.loading = 'lazy';
    ...
    link.textContent = isCurated ? 'View this post on Facebook ↗' : 'View the Page on Facebook ↗';
  ...
  function resizeEmbeds() {
    frames.forEach(item => {
      const width = Math.max(180, Math.min(500, Math.floor(item.panel.getBoundingClientRect().width || 350)));
      if (item.width === width) return;
      item.width = width;
      const params = new URLSearchParams({ href: item.url, width: String(width) });
      if (isCurated) {
